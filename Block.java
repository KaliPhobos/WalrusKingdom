...
public class Block {
	...
	public static int getPictureIDfromID(long _id) {
		...
		try {
			...
			General.DebugLog("komischer Schei�");
		}
		return _PictureID;
	}
	...
	public static void AddNew(Block NewBlock) {
        for(int i_AddNew=0; i_AddNew<Blocks.length; i_AddNew++) {
            if(getNameFromPos(i_AddNew).equals("")) {
                ...
                General.DebugLog("Block created: �"+i_AddNew+"       id="+NewBlock.id+"      name="+NewBlock.name+"      isSolid="+NewBlock.isSolid+"        PictureID="+NewBlock.PictureID+"      TriggerID="+NewBlock.TriggerID);
                break;
            }
        }
    }
    public static void AddNew(int _id, String _name, boolean _isSolid, int _PictureID, int _TriggerID) {
        for(int i_AddNew=0; i_AddNew<Blocks.length; i_AddNew++) {
            if(getNameFromPos(i_AddNew).equals("")) {
                ...
                General.DebugLog("Block created: �"+i_AddNew+"       id="+_id+"      name="+_name+"      isSolid="+_isSolid+"        PictureID="+_PictureID+"      TriggerID="+_TriggerID);
                break;
            }
        }
    }
